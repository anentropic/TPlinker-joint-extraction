"""
https://github.com/131250208/TPlinker-joint-extraction/blob/master/preprocess/BuildData.ipynb
but as a usable script
"""
import argparse
import json
import logging
...
def preprocess(config):
    ...
    if ori_format != "tplinker":  # if tplinker, skip transforming
        for file_name, data in file_name2data.items():
            ...
            if "test" in file_name:
                data_type = "test"
            data = preprocessor.transform_data(
        ...
        for sample in tqdm(data, desc="check tok spans"):
            ...
            for ent in sample["entity_list"]:
                ...
                if extr_ent(text, tok_span, tok2char_span) != ent["text"]:
                    span_error_memory.add(
                        "extr ent: {}---gold ent: {}".format(
                            extr_ent(text, tok_span, tok2char_span), ent["text"]
                        )
                    )
            ...
            for rel in sample["relation_list"]:
                ...
                if extr_ent(text, subj_tok_span, tok2char_span) != rel["subject"]:
                    span_error_memory.add(
                        "extr: {}---gold: {}".format(
                            extr_ent(text, subj_tok_span, tok2char_span), rel["subject"]
                        )
                    )
                if extr_ent(text, obj_tok_span, tok2char_span) != rel["object"]:
                    span_error_memory.add(
                        "extr: {}---gold: {}".format(
                            extr_ent(text, obj_tok_span, tok2char_span), rel["object"]
                        )
                    )
    ...
    for file_name, data in file_name2data.items():
        ...
        if "relation_list" in data[0]:  # train or valid data
            ...
            if config["add_char_span"]:
                data, miss_sample_list = preprocessor.add_char_span(
    ...
    json.dump(
        data_statistics,
        open(data_statistics_path, "w", encoding="utf-8"),
        ensure_ascii=False,
        indent=4,
    )
    ...
    if config["encoder"] in {
        ...
        json.dump(
            token2idx, open(dict_path, "w", encoding="utf-8"), ensure_ascii=False, indent=4
        )
        logging.info(
            "token2idx is output to {}, total token num: {}".format(
                dict_path, len(token2idx)
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default="build_data_config.yaml", help="config file"
    )
    parser.add_argument(
        "--exp-name", type=str, help="(config override) experiment name"
    )
    parser.add_argument(
        "--data-in-dir", type=str, help="(config override) input data dir"
    )
    parser.add_argument(
        "--data-out-dir", type=str, help="(config override) output data dir"
    )
    parser.add_argument(
        "--bert-path", type=str, help="(config override) local BERT path/HF model name"
    )
    ...
    if args.data_in_dir is not None:
        config["data_in_dir"] = args.data_in_dir
    if args.data_out_dir is not None:
        config["data_out_dir"] = args.data_out_dir
